...
def find_direct_dependencies(file_path: PathSrc) -> Set[PathHeader]:
	...
	direct_dependencies: Set[PathHeader] = set()
	with open(file_path, "r") as file:
		...
		for match in re.finditer(INCLUDE_RE, file.read(), re.MULTILINE):
			...
			if not os.path.isfile(included_file_path):
				...
				continue
			if not os.path.splitext(included_file_path)[1] == ".h":
				...
				print_error("Include warning",
					...
					"which is not a header file ending with `.h`.")
			direct_dependencies.add(included_file_path)
	return direct_dependencies

def find_dependencies(file_path: PathSrc) -> Set[PathHeader]:
	...
	dependencies: Set[PathHeader] = set()
...
class TuFileScope:
	...
	def __init__(self, c_file_path: PathC):
		self.c_file_path = c_file_path
		self.direct_dependencies = find_direct_dependencies(c_file_path)
		self.dependencies = find_dependencies(c_file_path)

	def files(self):
		return (self.c_file_path, *self.dependencies)
