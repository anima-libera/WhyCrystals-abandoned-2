...
def clear() -> None:
	...
	if os.path.isfile(embedded_c_file_path):
		print_verbose(f"Deleting the generated c file \"{embedded_c_file_path}\".")
		os.remove(embedded_c_file_path)
...
def main() -> None:
	...
	if not options.dont_build:
		...
		# the generated c file will be compiled if it is modified by the embedding.
		handle_embedding()
