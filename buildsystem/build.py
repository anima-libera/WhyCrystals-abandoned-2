...
def get_src_file_date_table() -> Dict[PathSrc, float]:
	...
	for file_path in walk_file_paths(SRC_DIR):
		if os.path.splitext(file_path)[1] in (".c", ".h"):
...
def get_unused_obj_id(tu_data_table: Dict[PathC, TuData]) -> ObjId:
...
def build(options: Options) -> bool:
	...
	c_file_paths: List[PathC] = list(filter(
		lambda file_path: os.path.splitext(file_path)[1] == ".c",
		src_file_date_table.keys()))
	tu_file_scope_table = {c_file_path: TuFileScope(c_file_path)
	...
	c_file_paths_to_build = []
	for tu_file_scope in tu_file_scope_table.values():
		c_file_path = tu_file_scope.c_file_path
		# Make sure every translation unit has an object id.
		if c_file_path not in tu_data_table:
			tu_data_table[c_file_path] = TuData(get_unused_obj_id(tu_data_table), dict())
			obj_id = tu_data_table[c_file_path].obj_id
			print_verbose(f"Assigned \"{c_file_path}\" to object id {obj_id:04d}.")
		tu_data = tu_data_table.get(c_file_path, None)
		...
		for src_file_path in tu_file_scope.files():
			...
			if old_src_file_date < src_file_date_table[src_file_path]:
				c_file_paths_to_build.append(c_file_path)
				break

	if c_file_paths_to_build:
		print_verbose("Compiling required for the following c files:")
		for c_file_path in c_file_paths_to_build:
			print_verbose(f"- \"{c_file_path}\"")
	for c_file_path in c_file_paths_to_build:
		tu_data = tu_data_table[c_file_path]
		obj_file_path = get_obj_file_path(build_mode_id, tu_data.obj_id)
		print_verbose(f"Compiling \"{c_file_path}\" into \"{obj_file_path}\":")
		formatted_compilation_command = build_mode.compilation_command.format(
			c = c_file_path, obj = obj_file_path)
		...
		if compilation_exit_status != 0:
			...
			tu_data_table[c_file_path].file_dates = {
				src_file_path: src_file_date_table[src_file_path]
				for src_file_path in tu_file_scope_table[c_file_path].files()}
	...
	if liking_required:
		all_obj_file_paths = list(map(
			lambda c_file_path: get_obj_file_path(
				build_mode_id, tu_data_table[c_file_path].obj_id),
			c_file_paths))
